...
const ViewHabit = ({ route, navigation }) => {
    ...
    const datesCalculation = (weekday, current, end) => {
        ...
        let upcomingDayFormatted = moment(upcomingDay).local().format("YYYY-MM-DD");
        while (moment(upcomingDayFormatted).isSameOrBefore(end)) {
            calculatedDates.push(upcomingDayFormatted);
            upcomingDayFormatted = moment(upcomingDayFormatted).add(7,'days').format("YYYY-MM-DD")
        }
        console.log("calculated dates are", calculatedDates);
